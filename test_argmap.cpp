...
TEST_F(TestArgMapSampleFile, documentationShownIfHelpSelectedFromFile)
{
  ...
  EXPECT_THROW(amap.parse(filepath), std::runtime_error);
}
...
TEST(TestArgMap, settingSameVariableTwice)
{
  ...
  EXPECT_THROW(amap.arg("bob", opts.arg1, "message string"), std::logic_error);
}
...
TEST_F(TestArgMapCmdLine, changingSeparatorCmdLine)
{
  ...
  ArgMap()
      .separator(ArgMap::Separator::COLON)
      .arg("alice", opts.arg1, "message string")
      .arg("bob", opts.arg2, "message string")
      .arg("chris", opts.arg3, "message string")
      .parse(argc, argv);
...
TEST_F(DeathTestArgMapCmdLine, wrongSeparatorNonWhitespaceCaseCmdLine)
{
  ...
  ArgMap amap;
  amap.separator(ArgMap::Separator::EQUALS)
      .arg("alice", opts.arg1, "message string")
      .arg("bob", opts.arg2, "message string")
      .arg("chris", opts.arg3, "message string");
...
TEST_F(DeathTestArgMapCmdLine, wrongSeparatorWhitespaceCaseCmdLine)
{
  ...
  ArgMap amap;
  amap.separator(ArgMap::Separator::WHITESPACE)
      .arg("alice", opts.arg1, "message string")
      .arg("bob", opts.arg2, "message string")
      .arg("chris", opts.arg3, "message string");
...
TEST_F(TestArgMapSampleFile, fileDoesNotExist)
{
  ...
  EXPECT_THROW(amap.parse("aaaa.bbb"), std::runtime_error);
}
...
TEST_F(TestArgMapSampleFile, toggleArgsFromFile)
{
  ...
  EXPECT_THROW(amap.parse(filepath), std::logic_error);
}
...
TEST_F(TestArgMapCmdLine, requiredToggleArgsAreOptionalCmdLine)
{
  ...
  // For TOGGLE_TRUE
  EXPECT_THROW(amap1.arg("-t", opts.toggle_t), std::logic_error);

  ArgMap amap2;
  amap2.required().toggle(false);

  // For TOGGLE_FALSE
  EXPECT_THROW(amap2.arg("-f", opts.toggle_f), std::logic_error);
}
...
TEST_F(TestArgMapCmdLine, onlyVariableNumbersOfOptionalPositional)
{
  ...
  ArgMap().dots(opts.dots, "file").parse(argc, argv);
...
TEST(TestArgMap, emptyNameOfArg)
{
  ...
  EXPECT_THROW(amap.arg("", i, ""), std::logic_error);
}
...
TEST(TestArgMap, whitespaceInNameOfArg)
{
  ...
  ArgMap amap1;
  // At beginning
  EXPECT_THROW(amap1.arg(" alice", i, ""), std::logic_error);

  ArgMap amap2;
  // In middle
  EXPECT_THROW(amap2.arg("ali ce", i, ""), std::logic_error);
...
TEST_F(TestArgMapCmdLine, readInNTLVectorCmdLine)
{
...
TEST_F(TestArgMapSampleFile, readInNTLVectorFromFile)
{
  std::ostringstream oss;
  oss << "alice=[1 2]\n";
